...
from bear.env_manager.worker import EnvWorker, DummyEnvWorker, SubprocEnvWorker, \
    RayEnvWorker


class BaseEnvManager(gym.Env):
    ...
    ::
        ...
        env_num = 8
        envs = DummyEnvManager([lambda: gym.make(task) for _ in range(env_num)])
    ...
    All of the EnvManager must inherit :class:`~bear.env_manager.BaseEnvManager`.
    ...
    def __init__(
        ...
        # A EnvManager contains a pool of EnvWorkers, which corresponds to
...
class DummyEnvManager(BaseEnvManager):
...
class SubprocEnvManager(BaseEnvManager):
...
class ShmemEnvManager(BaseEnvManager):
...
class RayEnvManager(BaseEnvManager):
